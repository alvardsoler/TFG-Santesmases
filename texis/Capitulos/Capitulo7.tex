...
\subsection{Unity + Vuforia}
\label{cap7:sub:unityvuforia}
...
\label{cap7:sub:salsa}

Gracias al paquete de Unity de animación por voz, SALSA lip sync, el proceso de sincronizar el audio con la animación del discurso del modelo ha sido algo muy sencillo. Dicha animación consta de tres componentes:
...
\label{cap7:sub:vozme}

Además del modelo utilizado, también hubo que crear los audios que narran el hilo argumental de nuestro juego. Para esta tarea, se generarón los audio con la herramienta vozme.com \footnote{http://vozme.com/index.php?lang=es} que convierte a mp3, con una voz un ``robótica'', un texto dado.

\section{Persistencia de puntuaciones}
%-------------------------------------------------------------------
\label{cap7:sec:puntuaciones}
