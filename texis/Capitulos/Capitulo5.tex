...
Space Invaders está ampliamente extendido en la cultura popular ya que es uno de los grandes clásicos (Fig: \ref{fig:inaveders1}), por eso hemos considerado acertado incluirlo en nuestro proyecto.
...
El juego se compone de una única escena que contiene todo el juego  (Fig: \ref{fig:invaders2}). Básicamente se compone de:
...
Pasamos a explicar qué componentes componen el juego y para qué los utilizamos.

\begin{enumerate}
\item{Defense}
	\begin{itemize}
	\item {Gestiona las defensas del usuario. Marca el color de inicio y el de final que debe tener la defensa para calcular los colores intermedios. Además gestiona las colisiones.  Va añadido al GameObject de las defensas.}
	\end{itemize}
\item{Projectile}
	\begin{itemize}
...
\item{Enjambre}
	\begin{itemize}
	\item {Se encarga de gestionar la inicialización del Enjambre y de sus invasores (colocándolos en la posición que les corresponda en función de cuántos sean y cuántas filas queremos que haya) y el movimiento del Enjambre (del que ``cuelgan'' los invasores), así como la escala de los invasores. Además contiene la información para saber si se han eliminado a todos los invasores o no. Va añadido a un GameObject vacío llamado Enjambre, el cual cuelga del detector de texto y del cual cuelgan los invasores.}
	\end{itemize}
\item{GameManager}
	\begin{itemize}
	\item {Es la clase que gestiona el juego en sí. Es un singleton y se le llama desde la mayoría de los otros scripts. Gestiona la interfaz de usuario, mostrando mensajes y los puntos cuando empieza el juego, además de cuando se puede disparar, etcétera. Es componente del GameManager, un GameObject vacío también.}
	\end{itemize}
\item{Invader}
	\begin{itemize}
	\item {Lógica del invasor. Gestiona los disparos de los invasores, la muerte de éstos y el sonido que hacen al ser destruidos. Va añadido al GameObject Invader.}
	\end{itemize}
\item{TextInformaticaTrackableEventHandler}
 	\begin{itemize}
	\item {Implementación propia de la clase ITrackableEventHandler de Vuforia. Va asociada al Text de RA. Al ``encontrarse'' y si no está instanciado ya (es decir, que no se ha ``encontrado'' varias veces), le dice al GameManager que comience el juego, indicándole dónde están el Enjambre y las Defensas en relación al Text.}
	\end{itemize}
\item{TextTimer}
	\begin{itemize}
	\item {De manera muy sencilla destruye el texto (de la interfaz gráfica) al que está asociado al pasar un tiempo dado una vez se ha habilitado. Lo utilizamos para mostrar los mensajes de texto de información previa, avisando al jugador cuando debe comenzar a disparar. Va asociado a GameObjects de texto.}
	\end{itemize}
\end{enumerate}
